						...
						{membershipsValue.failed ? (
							...
							<ul class="max-h-64 overflow-y-auto text-sm" aria-labelledby="account-switcher-button">
								{membershipsValue.map((account) => (
									<li key={account.id}>
										<Link href={`/${account.id}/`} onClick$={() => hideDropdown('account-switcher-menu')} class={['hover:bg-kumo-tint block truncate rounded-md px-2 py-1.5', account.id === currentAccountId.value ? 'text-kumo-brand font-medium' : 'text-kumo-default']}>
											{account.name}
										</Link>
									</li>
								))}
